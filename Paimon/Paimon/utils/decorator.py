    ...
    def wrapper(func):
        ...
        async def wrapped(**kwargs):
            ...
            try:
                ...
                await get_bot().send(event, f'ATRI可能被风控，也可能是没有该图片资源，消息发送失败')
            except JSONDecodeError:
                await get_bot().send(event, 'ATRI获取信息失败，重试一下吧')
            except FileNotFoundError as e:
                file_name = re.search(r'\'(.*)\'', str(e)).group(1)
                file_name = file_name.replace('\\\\', '/').split('/')
                file_name = file_name[-2] + '\\' + file_name[-1]
                await get_bot().send(event, f"ATRI缺少{file_name}资源，请联系开发者补充")
            except Exception as e:
                await get_bot().send(event, f'ATRI好像出了点问题，{e}')
